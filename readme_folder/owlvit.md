...
if __name__ == "__main__":
    ...
    parser.add_argument("--file_tag", type=str, required=True, help="A tag for the output filename (e.g., 'v2', 'ensemble').")
    ...
    if args.export_image_encoder:
        filename = f"{args.file_tag}_image_encoder.onnx"
    ...
    if args.export_text_encoder:
        filename = f"{args.file_tag}_text_encoder.onnx"
        path = os.path.join(output_dir, filename)
    ...
    if args.export_prediction_head:
        filename = f"{args.file_tag}_prediction_head.onnx"
        path = os.path.join(output_dir, filename)
